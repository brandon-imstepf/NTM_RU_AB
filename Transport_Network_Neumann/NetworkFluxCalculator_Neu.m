...
n0 = @(B) B;   % IC: n(x=0) = B
n_ss_presyn = @(B, N_i, idx) ode15s(@(x, n) ode_ss_n(x, B, n, diff_n, N_i, idx), ...
...
n_ss_ais = @(B, N_i, idx, lambda1_i, lambda1_j) ...
    ode15s(@(x, n) ode_ss_n(x, B, n, diff_n * lambda_fun(lambda1_i, lambda1_j, x), N_i, idx), ...
           [L1_new, L1_new + L_ais_new], n_ss_presyn(B, N_i, idx, x1), options);
...
n_ss_axon = @(B, N_i, idx, lambda1_i, lambda1_j, gamma1_i, gamma1_j) ...
    ode15s(@(x, n) ode_ss_axon(x, B, gamma1_i, gamma1_j, n, N_i, idx), ...
           [L1_new + L_ais_new, L1_new + L_int_new - L_syn_new], ...
           n_ss_ais(B, N_i, idx, lambda1_i, lambda1_j, x2), options);
n_ss_axon = @(B, N_i, idx, lambda1_i, lambda1_j, gamma1_i, gamma1_j, x) ...
    deval(n_ss_axon(B, N_i, idx, lambda1_i, lambda1_j, gamma1_i, gamma1_j), x);
...
parfor i = 1:nroi
    ...
    if ~isempty(tau_x0_i)  % skip if region i has no incoming connections
        ...
        % ------------------------------------------------------------------
        netflux_L_temp = mu_r_L .* n_ss_axon(n_ss0_temp, tau_x0_i, idx_netw, ...
                                               lambda1_x0_i, lambda1_xL_i, ...
                                               gamma1_x0_i, gamma1_xL_i, x3) ...
                         - mu_u_L .* tau_xL_i;
...
% =========================================================================
    ...
    function nprime = ode_ss_axon(x, B, gamma1_i, gamma1_j, n, N_i, idx)
        nprime = 1 ./ (ip.Results.frac * diff_n) .* ...
                 ((1 - ip.Results.frac) .* ...
                  ((v_a .* (1 + ip.Results.delta .* n) .* ...
                    (1 - (gamma1_fun(gamma1_i, gamma1_j) .* ip.Results.epsilon .* n.^2) ./ ...
