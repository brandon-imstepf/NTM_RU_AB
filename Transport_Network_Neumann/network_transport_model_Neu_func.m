...
% --- Amyloid Beta terms ---
Bsol = 0; % Initialize Bsol, will be solved in section ~8.5
Abeta = 0; % Initalize, will be solved in section ~8.5
r_ = 1;
...
kappa_mu_u_ = 1; % For AB coupling w/ mu_u_hat
delta_hat_ = 1; % For AB Coupling w/ Delta (Retrograde)
kappa_delta_ = 1; % For AB coupling w/ delta_hat
%sigma_hat_ = 1; % AB Coupling w/ Sigma (Release but new term)
%kappa_sigma_ = 1; % AB coupling w/ sigma_hat
...
addParameter(ip, 'kappa_mu_u',        kappa_mu_u_,         validScalar);
%addParameter(ip, 'delta_hat',         delta_hat_,          validScalar);
addParameter(ip, 'kappa_delta',       kappa_delta_,        validScalar);
%addParameter(ip, 'gamma_hat',         gamma_hat_,          validScalar);
addParameter(ip, 'kappa_gamma',       kappa_gamma_,        validScalar);
addParameter(ip, 'kappa_epsilon',     kappa_epsilon_,      validScalar);
parse(ip, varargin{:});
...
% ==========
% Section 8.5: Pre-compute Amyloid-Beta
%
% We will calculate Amyloid-Beta using proposed expression:
%   dB_i/dt = r*B_i(1-B_i) + sum_j[C_ij(B_j - B_i)]
% where rB_i(1-B_i) is local logistic growth
% sum_j[C_ij(B_j - B_i)] is the Graph Laplacian over the proximity graph
% weights C_ij.
%
% ===========
B0 = 0.05*ones(nroi,1); % Initial conditions for Bnot. This assumes globally diffuse.
[~,Bsol] = ode45(@(t,B) ip.Results.r*B .*(1-B) + ip.Results.Dif*( Conn*B - sum(Conn,2).*B ) ,t,B0); % Logistic growth + Laplacian
Abeta = Bsol.'; % Make same shape as Gamma1
